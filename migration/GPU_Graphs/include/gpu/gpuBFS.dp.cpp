                                ...
                                volatile int *base_offset)
{
 ...
 void fine_gather(const int* const column_index, int* const distance,
                             const int iteration, int * const out_queue, 
                             int* const out_queue_count,int r, int r_end,
                             const sycl::nd_item<3> &item_ct1,
                             volatile int *sums, int *comm,
                             volatile int *base_offset)
{
        prescan_result rank = block_prefix_sum(r_end - r, item_ct1, sums);
	...
	while ((rank.total - cta_progress) > 0)
	{
		...
		while((rank.offset < cta_progress + BLOCK_SIZE) && (r < r_end))
		{
			...
			r++;
                ...
                /*
                DPCT1118:7: SYCL group functions and algorithms must be
                encountered in converged control flow. You may need to adjust
                the code.
                */
                /*
		...
		int valid = 0;
                ...
                {
                        neighbor = column_index[comm[item_ct1.get_local_id(2)]];
                        // Look up status
            ...
            if ((distance[neighbor] == -1))
            {
                ...
                /*
                DPCT1118:8: SYCL group functions and algorithms must be
                encountered in converged control flow. You may need to adjust
                the code.
                */
                /*
                ...
                */
                ...
                /*
                DPCT1118:9: SYCL group functions and algorithms must be
                encountered in converged control flow. You may need to adjust
                the code.
                */
                /*
		...
		cta_progress += BLOCK_SIZE;
                /*
                DPCT1118:10: SYCL group functions and algorithms must be
                encountered in converged control flow. You may need to adjust
                the code.
                */
                /*
